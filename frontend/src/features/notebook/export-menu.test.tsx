...
async function openMenu() {
  ...
  return screen.findByRole('menuitem', { name: /Copy to clipboard/ });
}
...
describe('ExportMenu (Phase 6, PRD §14, §16)', () => {
  it('offers the three outputs; download and print point where they should', async () => {
    ...
    await openMenu();
    expect(screen.getByRole('menuitem', { name: /Download Markdown/ })).toHaveAttribute(
      'href',
      '/api/spaces/space-1/notebook/export.md',
    );
    expect(screen.getByRole('menuitem', { name: /Print/ })).toHaveAttribute('href', '/spaces/space-1/notebook/print');
  });
  ...
  it('copies the server Markdown to the clipboard and announces it', async () => {
    ...
    expect(await screen.findByText('Notebook copied to the clipboard')).toBeInTheDocument();
  });
